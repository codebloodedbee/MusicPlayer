package com.example.hizkeel.musicplayer.Activity
...
import com.example.hizkeel.musicplayer.Adapter.ViewPagerAdapter
import com.example.hizkeel.musicplayer.DB.FavoritesOperations
import com.example.hizkeel.musicplayer.Fragments.AllSongFragment.createDataParse
...
import com.example.hizkeel.musicplayer.Model.SongsList
import com.example.hizkeel.musicplayer.R
import java.util.*

class MainActivity : AppCompatActivity(), View.OnClickListener, createDataParse, FavSongFragment.createDataParsed, CurrentSongFragment.createDataParsed {
    ...
    private fun grantedPermission() {
        if (ContextCompat.checkSelfPermission(this@MainActivity,
            ...
            ActivityCompat.requestPermissions(this@MainActivity, arrayOf(Manifest.permission.READ_EXTERNAL_STORAGE), MY_PERMISSION_REQUEST)
            if (ActivityCompat.shouldShowRequestPermissionRationale(this@MainActivity,
                ...
                if (ContextCompat.checkSelfPermission(this@MainActivity,
                                Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                    val snackbar = Snackbar.make(mDrawerLayout!!, "Provide the Storage Permission", Snackbar.LENGTH_LONG)
                    snackbar.show()
                }
            }
        } else {
            ...
            R.id.menu_favorites -> {
                if (checkFlag) if (mediaPlayer != null) {
                    favFlag = if (favFlag) {
                        ...
                        val favList = SongsList(songList!![currentPosition].title,
                                songList!![currentPosition].subTitle, songList!![currentPosition].path)
            ...
            R.id.img_btn_previous -> if (checkFlag) {
                if (mediaPlayer!!.currentPosition > 10) {
                    if (currentPosition - 1 > -1) {
                        attachMusic(songList!![currentPosition - 1].title, songList!![currentPosition - 1].path)
                        currentPosition = currentPosition - 1
                    } else {
                        attachMusic(songList!![currentPosition].title, songList!![currentPosition].path)
                    }
                } else {
                    attachMusic(songList!![currentPosition].title, songList!![currentPosition].path)
                }
    ...
    override fun fullSongList(songList: ArrayList<SongsList>, position: Int) {
    ...
    override fun getSong(): SongsList {
        currentPosition = -1
        return currSong!!
    }

    override fun getPlaylistFlag(): Boolean {
        return playlistFlag
    }
